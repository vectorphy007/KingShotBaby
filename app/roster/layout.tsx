    ...
    <div className="bg-[#0B0F14]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
          ...
          {/* Context Panel (previously Command sidebar) */}
          <aside className="w-full max-w-sm bg-secondary border border-slate-700 rounded-xl p-6 space-y-6">
            <div>
              <h2 className="text-2xl font-black text-[#F59E0B] tracking-[0.2em] uppercase italic">Command</h2>
              <div className="h-1 w-12 bg-[#F59E0B] mt-2 shadow-[0_0_10px_#F59E0B]"></div>
            </div>

            <div className="space-y-6">
              <div>
                <p className="text-[10px] uppercase text-[#9CA3AF] mb-4 tracking-[0.3em] font-bold">Main Operations</p>
                <ul className="space-y-2">
                  {navItems.map((item) => (
                    <li key={item.path}>
                      <Link
                        href={item.path}
                        className={`block py-2 px-3 rounded-md text-sm transition-all border-l-2 ${
                          isActive(item.path)
              ...
              <div>
                <p className="text-[10px] uppercase text-[#9CA3AF] mb-4 tracking-[0.3em] font-bold">Tactical (1:3:3:3)</p>
                <ul className="space-y-2">
